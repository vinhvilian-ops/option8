...
# The big news div closes after `</button>`. BUT it was missing the </div>s.
# We will just extract until `</button>`
big_btn = '</button>'
big_news_end = weekly_block.find(big_btn, big_news_start) + len(big_btn)
...
small_news_end = weekly_block.find('})}') + 3
small_news_content = weekly_block[small_news_start:small_news_end]

with open('/tmp/9_banners.tsx', 'r') as fb:
    banners = fb.read().strip()
    
# Remove the extra `</div>` at the end of banners if it exists
if banners.endswith('</div>'):
    banners = banners[:-6]

# Rebuild the section cleanly
